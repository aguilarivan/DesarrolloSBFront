...
const VendedoresPage = () => {
  ...
  const filteredVendors = vendors.filter((vendor) => vendor.nombre.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="space-y-6">
      ...
      <VendorTable vendors={filteredVendors} setVendors={setVendors} />

      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <CreateVendorForm onCancel={() => setIsModalOpen(false)} onConfirm={handleCreateVendor} />
      </Modal>
    </div>
  );
};
